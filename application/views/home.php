                ...
                <div class="container">
                    <div class="row">
                        <div class="col-xs-12">
                            ...
                            <p class="parallax-element-second">  
                            <?php $siparisBilgi = $this->HomeModel->masaId("siparis", array("masa_no" => $this->session->userdata("masaid"))); 
                            if(!$siparisBilgi){
                                echo "Hoşgeldiniz!";
                            }
                            elseif($siparisBilgi->durum == 1){
                                echo "Siparişiniz Alındı";
                            }elseif($siparisBilgi->durum == 2){
                                echo "Siparişiniz Hazırlanıyor";
                            }elseif($siparisBilgi->durum == 3){
                                echo "Siparişiniz Masada";
                            }
                            ?> </p>
                            <p class="parallax-element-second"> Yemek istediklerinizi sepete ekleyin ve sipariş verin. Siparişiniz hazır olduğunda masanıza gelir.</p>
                        </div>
                    </div>

                    <div>
                    <a href="<?php echo base_url("Home/masaac"); ?>" class="btn btn-light-warning btn-sm font-weight-bold mr-2" data-id="<?php echo $this->session->userdata("masaid"); ?>" data-masa='ac'>Menüye Git</a>
                    </div>
                    ...
                    <script>
                        $(document).on('click', '[data-masa="ac"]', function() {
                            ...
                            $.ajax({
                                url: "<?php echo base_url("Home/masaac"); ?>",
                                type: 'POST',
                                data: {
                                    id: id,
                                },
